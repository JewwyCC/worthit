...
def test_youtube_accessibility():
    """Test if YouTube is accessible with different user agents"""
    ...
    for i, user_agent in enumerate(user_agents, 1):
        try:
            ...
            if response.status_code == 200:
                print(f"✅ YouTube homepage (UA {i}): Accessible")
            else:
                print(f"⚠️ YouTube homepage (UA {i}): Status {response.status_code}")
            
            # Small delay between requests
            time.sleep(1)
            
        except Exception as e:
            print(f"❌ YouTube homepage (UA {i}): Failed - {e}")
    
    try:
        # Test YouTube API endpoint
        headers = {'User-Agent': random.choice(user_agents)}
        response = requests.get("https://www.googleapis.com/youtube/v3/search", headers=headers, timeout=10)
...
def test_user_agent_effectiveness():
    """Test different user agents to see which work best"""
    ...
    print(f"   📊 Success rate: {len(successful_agents)}/{len(user_agents)} ({len(successful_agents)/len(user_agents)*100:.0f}%)")
...
def test_with_proxy():
    ...
    try:
        ...
        if len(proxy_manager) >= 2:
            ...
            # Test with proxy and user agent
            user_agent = 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/120.0.0.0 Safari/537.36'
            headers = {'User-Agent': user_agent}
            
            # Test with proxy
            response = requests.get("https://httpbin.org/ip", proxies=proxy_dict, headers=headers, timeout=10)
            proxy_ip = response.json().get('origin', 'Unknown')
            print(f"   ✅ Proxy IP: {proxy_ip}")
            
            # Test YouTube with proxy + user agent
            response = requests.get("https://www.youtube.com", proxies=proxy_dict, headers=headers, timeout=10)
            if response.status_code == 200:
                print(f"   ✅ YouTube via proxy + user agent: Accessible")
            else:
                print(f"   ⚠️ YouTube via proxy + user agent: Status {response.status_code}")
